...
describe('Manager', () => {
  describe('InitializeJs', () => {
    ...
    test("Can set name attribute with constructor", () => {
      const name = "Tucker";
      const cb = new manager(name, "Manager", 7, "Tucker@gmail.com", 144, "tbeauchamp");
      expect(cb.name).toBe(name);
    });
    test("Can set role attribute with constructor", () => {
      const role = "Manager";
      const cb = new manager(role, "Tucker", 7, "Tucker@gmail.com", 144, "tbeauchamp");
      expect(cb.getRole()).toBe(role);
    });
    test("Can set id attribute with constructor", () => {
      const id = 7;
      const cb = new manager("Tucker", id, "Manager", "Tucker@gmail.com", 144, "tbeauchamp");
      expect(cb.id).toBe(id);
    });
    test("Can set email attribute with constructor", () => {
      const email = "Tucker@gmail.com";
      const cb = new manager("Tucker", "Manager", email, 7,  144, "tbeauchamp");
      expect(cb.email).toBe(email);
    });
    test("Can set officeNumber attribute with constructor", () => {
      const officeNumber = 144;
      const cb = new manager("Tucker", "Manager", 7, officeNumber, "Tucker@gmail.com", "tbeauchamp");
      expect(cb.officeNumber).toBe(officeNumber);
    });
    test("Can set github attribute with constructor", () => {
      const github = "tbeauchamp";
      const cb = new manager("Tucker", "Manager", 7, "Tucker@gmail.com", github, 144);
  ...
  describe("getInfo", () => {
    ...
    test("Does getRole() method get role?", () => {
      const role = "Manager";
      const cb = new manager("Tucker", role, 7, "Tucker@gmail.com", 144, "tbeauchamp");
      expect(cb.getRole()).toBe(role);
    });
